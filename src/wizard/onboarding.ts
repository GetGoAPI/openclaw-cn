...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      "安全警告 — 请阅读。",
      "",
      "OpenClaw 属于业余项目，目前仍处于 beta 测试阶段。预期会有一些不完善的地方。",
      "默认情况下，OpenClaw 是一个私人代理：只有一个受信任的运营者边界。",
      "如果启用了相关工具，这个机器人的权限可以读取文件并执行操作。",
      "恶意的提示词（prompt）可能会欺骗它执行不安全的操作。",
      "",
      "OpenClaw 默认不是为充满敌意的多租户环境设计的。",
      "如果多个用户可以向一个启用了工具的代理发送消息，那么他们将共享该代理被授予的工具权限。",
      "",
      "如果您对安全加固和访问控制没有把握，请不要运行 OpenClaw。",
      "在启用工具或将其暴露到互联网之前，请寻求有经验的人员的帮助。",
      "",
      "推荐的安全基线：",
      "- 配对/白名单 + 「提及（@）」门控。",
      "- 多用户/共享收件箱：分离信任边界（使用独立的网关/凭证，最好使用独立的操作系统用户/主机）。",
      "- 沙箱环境 + 最小权限工具。",
      "- 共享收件箱：隔离私信（DM）会话 (`session.dmScope: per-channel-peer`) 并且保持工具访问权限最小化。",
      "- 让机密信息（secrets）远离代理可触及的文件系统。",
      "- 对于拥有工具权限或运行在不受信任的收件箱中的任何机器人，请使用最强大的可用模型。",
      "",
      "请定期运行以下命令：",
  ...
  const ok = await params.prompter.confirm({
    message:
      "我已了解该程序默认为个人使用，如果在共享/多用户环境下使用需要锁定权限。是否继续？",
    initialValue: false,
  });
  if (!ok) {
